...
export async function scene1() {
  ...
  await sailorA.show({ x: 0.75, y: 0.10, layer: 2, scale: 0.6 });
  await sailorB.show({ x: 0.10, y: 0.20, layer: 2, scale: 0.6 });
  ...
  await sailorA.move({ x: 0.65, y: 0.18, scale: 0.6 });
  await sailorB.move({ x: 0.18, y: 0.30, scale: 0.6 });
  ...
  await sailorA.move({ x: 0.55, y: 0.25, scale: 0.6 });
  await sailorB.move({ x: 0.25, y: 0.35, scale: 0.6 });
  await sailorA.pose('e');
  await sailorB.pose('f');
  await sailorA.say('风有点大，别让帽子飞了。');
  await sailorB.say('好险，差点就掉了。');

  await neko.show({ x: 0.02, y: 0.02, layer: 3, scale: 0.5, opacity: 0.9 });
  await neko.pose('g');
  await sailorA.say('呀，小猫咪也出来散步。');
  await sailorB.say('好可爱，想摸摸它。');

  await sailorA.move({ x: 0.62, y: 0.28, scale: 0.6 });
  await sailorB.move({ x: 0.20, y: 0.28, scale: 0.6 });
  await sailorA.pose('g');
  await sailorB.pose('h');
  await sailorA.say('我们要不要邀请她一起？');
  await sailorB.say('好，正好人多可以点更多。');

  await akamafu.show({ x: 0.40, y: 0.15, layer: 2, scale: 0.6 });
  ...
  await sailorA.move({ x: 0.80, y: 0.12, scale: 0.6 });
  await sailorB.move({ x: 0.18, y: 0.22, scale: 0.6 });
  await akamafu.move({ x: 0.48, y: 0.18, scale: 0.6 });
  await neko.move({ x: 0.05, y: 0.05, scale: 0.52 });
  ...
  await sailorA.move({ x: 0.70, y: 0.26, scale: 0.6 });
  await sailorB.move({ x: 0.12, y: 0.30, scale: 0.6 });
  await akamafu.move({ x: 0.45, y: 0.20, scale: 0.6 });
  await neko.move({ x: 0.08, y: 0.08, scale: 0.55 });
